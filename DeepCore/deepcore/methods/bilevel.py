...
class MaskWeight(nn.Module):
    def __init__(self, num_train, num_class):
        ...
        #self.W[:256] += 0.6
        #self.W[256:] += 0.4
        #self.W = torch.rand(num_train)/10+0.45
...
class BiLevel(EarlyTrain):
    ...
    def run(self):
        ...
        # Setup optimizer
        print("selection_lr: ", self.args.selection_lr)
    ...
    def bilevel_optimization(self):
        ...
        self.outer_train_loader = torch.utils.data.DataLoader(self.dst_train, shuffle=True, drop_last=False, 
                                                batch_size=self.args.meta_batch, num_workers=self.args.workers, pin_memory=False)
    ...
    def bilevel_train(self, inner_loader, outer_loader, epoch, cur_lamda):
        ...
        for i, data in enumerate(inner_loader):
            ...
            #TODO: check if redundant parameter updated
            for j in range(self.args.meta_steps):
                ...
                if j != self.args.meta_steps-1:
                    # Using different meta-batch every param_updates
                    try:
                        inputs_val, targets_val, idxs_val = next(outer_loader_iter)
                    except:
                        outer_loader_iter = iter(outer_loader)
            ...
            #print("first", self.W.get_weight()[0:10])
            #print("second", self.W.get_weight()[256:266])
