...
class NaiveGreedy(optimizer):
    ...
    def select(self, gain_function, update_state=None, **kwargs):
        ...
        uncertain_indices = np.array([], dtype=np.int64)
        for i in range(sum(selected), self.budget):
            ...
            uncertain_indices = np.append(uncertain_indices, current_selection)

            greedy_gain[current_selection] = -np.inf
            if update_state is not None:
                update_state(np.array([current_selection]), selected, **kwargs)

        return self.index[uncertain_indices] #self.index[selected]
        ...
        for i in range(sum(selected), self.budget):
            ...
            while True:
                ...
                if last_max_element == cur_max_element:
                    ...
                    uncertain_indices = np.append(uncertain_indices, cur_max_element)
...
class StochasticGreedy(optimizer):
    ...
    def select(self, gain_function, update_state=None, **kwargs):
        ...
        for i in range(sum(selected), self.budget):
            ...
            uncertain_indices = np.append(uncertain_indices, subset[current_selection])
