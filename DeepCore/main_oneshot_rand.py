...
def main():
    ...
    for exp in range(start_exp, args.num_exp):
        ...
        n_rand = 2500
        rand_idx = np.random.choice(len(dst_train), n_rand)

        non_rand_idx = []
        for i in subset["indices"]:
            if i not in rand_idx:
                non_rand_idx.append(i)
        assert len(list(set(non_rand_idx)-set(rand_idx))) == len(non_rand_idx)

        print("n_grid: ", n_grid)
        for g in range(n_grid):
            indices = list(rand_idx) + non_rand_idx[:(g+1)*n_query-n_rand]
            ...
            if g == 0:
                macs, params = get_model_complexity_info(network, (args.channel, args.im_size[0], args.im_size[1]), as_strings=True, print_per_layer_stat=False, verbose=False)
            ...
            path = 'result/'+args.dataset+'/'+args.selection+'_w_rand.txt'
            if g == 0:
                f = open(path, 'w')
            else:
                f = open(path, 'a')
